...
@section('body')
    <div class="w-100 d-flex justify-content-center">
        <form class="w-50" method="GET" action="{{ route('livros.update', $book->id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="form-group mb-2">
                ...
                <input name="international_standard_book_number" class="form-control" maxlength="13" value="{{ $book->international_standard_book_number }}" />
            </div>

            <div class="form-group mb-2">
                <label class="form-label fw-semibold">Preço de custo:</label>
                <input name="cost_price" class="form-control" value="{{ $book->cost_price }}" />
            </div>

            <div class="form-group mb-4">
                <label class="form-label fw-semibold">Preço de venda:</label>
                <input name="sale_price" class="form-control" value="{{ $book->sale_price }}" />
            </div>
